...
%                                    * isFeasible - Fxn handle for constraintRevision.m fxn
...
ptarget   = .99999;                                                                      % Probability (target) of moving a member at least nTimes (e.g. 99%)
...
if ~sample.uniform
    dF= sample.discreteFactor;
    lambdaValues  = zeros(dF,sample.numChains);
    posNextPoints = zeros(size(points,1),dF,sample.numChains);
    nextProbs     = zeros(dF,sample.numChains);
    F             = zeros(dF,sample.numChains);
    dF            = dF - 1;
end
...
while (iterSample <= nSteps(end))
    ...
    while any(flags)
        ...
        for ix = find(flags)
            ...
            if all(abs(udir(:,ix))<=1e-7); continue; end
        ...
        % flags(flags) = (Lcord(flags) < 2*sample.epsTol) | ((Lcord(flags) - 2*sample.epsTol) < sample.bTol) | (posStep(flags) < 0) | (negStep(flags) > 0);
    end
    ...
    while any(flags)
        ...
        if sample.uniform
            ...
            lambdaValues(:,flags) = (0:dF)' .* Lcord(flags)/dF + negStep(flags);
            % lambdaValues(:,flags) = (0:dF)' .* (Lcord(flags) - 2*sample.epsTol)/dF + negStep(flags) + sample.epsTol;
            
            % Calculate points and evaluate pdf
            for i=find(flags)
            ...
            for i=find(flags)
                steps(i) = lambdaValues(find(F(:,i),1),i);
            end
        end

        nextPoint(:,flags) = mappingFxn(currPoint(:,flags),udir(:,flags),steps(flags));
        unfeasCounter = unfeasCounter + sum(~sample.isFeasible(nextPoint(:,flags)));
        if any(~sample.isFeasible(nextPoint(:,flags))) && feasFlag==0
            save("debug19.mat","nextPoint","currPoint","udir","F","lambdaValues","steps","flags","idx_next","posStep","negStep","points")
            feasFlag = 1;
        end
        nextPoint(:,flags) = sample.keepWithinBounds(nextPoint(:,flags));
        nextPoint(:,~sample.isFeasible(nextPoint(:,flags))) = currPoint(:,~sample.isFeasible(nextPoint(:,flags)));
        if sample.loopless
...
fprintf("Unfeasible points generated through the sampling: %1d \n",unfeasCounter)
