function [points,samplingTime,rejectionRate] = kernelHitAndRun(sample,numSamples,numDiscarted,stepsPerPoint,verbose,workerIdx)
...
%%%%%% CODE TAKEN FROM GENERAL HR %%%%%%
% Determine the dimension of the feasible space
Aeq = full(sample.S);
ix_fixed = (abs(sample.ub-sample.lb)==0);
if any(ix_fixed)
    Aeq(:,ix_fixed) = 0;
end
%%%%%% CODE TAKEN FROM GENERAL HR %%%%%%
...
P_N    = kernel*kernel';             % Build projection matrix onto null(S)
...
while sampleCount<numSamples
    ...
    % fprintf('rejectedCount: %d \n',rejectedCount)
    % fprintf('rejectionRate: %d \n',rejectionRate)
    ...
    %     save("debugVecs.mat","prevPoint","maxStepVec","minStepVec","distLb","distUb")
    %     return
    % end
    ...
    if (abs(minStep)<maxMinTol && abs(maxStep)<maxMinTol) || (minStep>maxStep) || (maxStep<0) || (minStep>0)
        ...
        %     disp("condicion:   4")
        % end
        % fprintf("we get too close: %d \n",rejectedCount)
        continue;
    end

    % fprintf("Llega aqui:   %d  \n",rejectedCount)
    ...
    % if (max(abs(sample.S*nextPoint - sample.b))>dTol)
    %     nextPoint = P_N*nextPoint + prevPoint;       % Testing if it works-> it doesnt
    %     fprintf("Llega a proyeccion:   %d  \n",rejectedCount)
    % end

    % Bring points to the boundary if they fall out
    if any((sample.ub-nextPoint)<0)
        overInd = (nextPoint>sample.ub);
        nextPoint(overInd) = sample.ub(overInd);
        fprintf("Llega aqui v1:   %d  \n",rejectedCount)
    end
    if any((nextPoint-sample.lb)<0)
        underInd = (nextPoint<sample.lb);
        nextPoint(underInd) = sample.lb(underInd);
        fprintf("Llega aqui v2:   %d  \n",rejectedCount)
    end
    ...
    looplessModel = 1; % TEMPORARY, TESTING
    if looplessModel
        counter   = counter+1;
        prevPoint = nextPoint;
        ...
        while true
            ...
            % Bring points to the boundary if they fall out
            if any((sample.ub(1:nVars)-nextPoint)<0)
                overInd = (nextPoint>sample.ub(1:nVars));
                nextPoint(overInd) = sample.ub(overInd);
            end
            if any((nextPoint-sample.lb(1:nVars))<0)
                underInd = (nextPoint<sample.lb(1:nVars));
                nextPoint(underInd) = sample.lb(underInd);
            end
