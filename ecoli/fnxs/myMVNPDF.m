function y = myMVNPDF(sample,prevPoint,nextPoint)
% Calculates ratio of mvnpdf function for two points.
X1 = prevPoint(sample.normalVars)';
X2 = nextPoint(sample.normalVars)';
Mu = sample.bigMu';
Sigma = sample.bigCov;
...
elseif ndims(Sigma) == 2
    ...
    elseif ~isequal(sz, [d d])
        ...
        if sigmaIsDiag
            ...
            xRinv = X0 / R;
            logSqrtDetSigma1 = sum(log(diag(R)));
        end
    end
...
elseif ndims(Sigma) == 3
    ...
    elseif sz(3) ~= n
        ...
        if sigmaIsDiag
            ...
            logSqrtDetSigma1 = zeros(n,1,'like',Sigma);
            for i = 1:n
                % Make sure Sigma is a valid covariance matrix
                ...
                xRinv(i,:) = X0(i,:) / R;
                logSqrtDetSigma1(i) = sum(log(diag(R)));
            end
        end
    end
...
elseif ndims(Sigma) == 2
    ...
    elseif ~isequal(sz, [d d])
        ...
        if sigmaIsDiag
            ...
            % Create array of standardized data, and compute log(sqrt(det(Sigma)))
            xRinv = X0 / R;
            logSqrtDetSigma2 = sum(log(diag(R)));
        end
    end
...
elseif ndims(Sigma) == 3
    ...
    elseif sz(3) ~= n
        ...
        if sigmaIsDiag
            ...
            R = sqrt(Sigma);
            xRinv = X0./R;
            logSqrtDetSigma2 = sum(log(R),2);
        else
            % Create array of standardized data, and vector of log(sqrt(det(Sigma)))
            xRinv = zeros(n,d,'like',internal.stats.dominantType(X0,Sigma));
            logSqrtDetSigma2 = zeros(n,1,'like',Sigma);
            for i = 1:n
                % Make sure Sigma is a valid covariance matrix
                [R,err] = cholcov(Sigma(:,:,i),0);
                if err ~= 0
                ...
                xRinv(i,:) = X0(i,:) / R;
                logSqrtDetSigma2(i) = sum(log(diag(R)));
            end
        end
    end
...
% The quadratic form is the inner products of the standardized data
quadform2 = sum(xRinv.^2, 2);

%%%%% CALCULATE RATIO %%%%%

% y = exp(-0.5*quadform2 - logSqrtDetSigma2 + 0.5*quadform1 + logSqrtDetSigma1);
y = -0.5*quadform2 + 0.5*quadform1;
% Dejarlo en log solo comparandolo y viendo si es <>0.
% Calcular el logaritmo de la distribucion uniforme cuando 0<1
% Now "y" is the ratio so it can regulate the difference of magnitude.
