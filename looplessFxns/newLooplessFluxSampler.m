function sample = newLooplessFluxSampler(model,options)
% Adapted version of Loopless Flux Sampler
%
...
% Commented to avoid conflict with "mets"
...
sample.rxns     = tempModel.rxns;
sample.numRxns  = numel(tempModel.lb);
sample.S        = tempModel.S;
sample.lb       = tempModel.lb;
sample.ub       = tempModel.ub;
sample.uTol     = 1e-12;                                                    % Direction tolerance
sample.bTol     = 1e-11;                                                    % Minimum allowed distance to closest constraint
sample.feasTol  = 1e-7;
% New fields (nonUniformADSB)
% sample = addFieldsToSample(sample,model,zeroRxns); % TEMP e coli
sample.uniform        = model.uniform;  % TEMP e coli
% sample.discreteFactor = model.discreteFactor;
sample.isFeasible = @(points) closa_constrainRevision(points,sample.S,sample.lb,sample.ub,sample.feasTol);  % TEMP e coli
% sample.densityFxn = @(points) testDensityFxn(points);
...
if ~sample.parallelFlag
    ...
    if strcmp(sample.algorithm,'ADSB')
        [sample.points,sample.samplingTime] = nonUniformADSB(sample);
