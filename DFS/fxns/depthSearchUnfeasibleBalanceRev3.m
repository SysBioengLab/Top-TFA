function depthSearchUnfeasibleBalanceRev3(model,directionSpace,constIdx,folderName)
% Finds unfeasible flux patterns based on mass balance constraints with two
% metabolits conected
%%%%%%%%%%%%%%%%%%%%%% Pedro Saa UQ 2015 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% mofified by Veronica Martinez January 2017
...
for i=1:length(check)       %%%added Vero
    if check(i)>constIdx       %%%added Vero to do not repeat the previous rules again
        ...
        [~,preactiveFluxes2] = find(Stoic~=0); %%%added Vero used to find the potential third reaction involved in a mass balance
        Stoic3=full(model.S(:,preactiveFluxes2)); %%%added Vero
        Stoic3([constIdx,check(i)],:)=zeros(2,size(Stoic3,2)); %%%added Vero, remove from the matrix the studied metabolite
        Stoic3(logical(Stoic3))=1;    %%%added Vero
        check2=find(sum(Stoic3,2)>1);
        for k=1:length(check2)
             if check2(k)>check(i)
                Stoic=sum(full(model.S([constIdx,check(i),check2(k)],:)));   %%%added Vero external reactions to that three metabolites
                ...
                if ~isempty(unfeasiblePatterns)
                    disp(['Unfeasible modes found in triple balance ',num2str(constIdx),'  ',num2str(check(i)),'  ',num2str(check2(k)),' = ',num2str(size(unfeasiblePatterns,1))]);
                    save([folderName,'/unbalancedTopology3_',num2str(constIdx),'_',num2str(check(i)),'_',num2str(check2(k)),'.mat']);
                end
            end
        end
    end
