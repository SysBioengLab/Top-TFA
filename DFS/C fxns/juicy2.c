...
short *cfgs;
static const int  DEFAULT_CFG_SIZE = 100000;
static const float SCALING_FACTOR = 1.20f;
int numCfgs;
int cfgArrSize;
int numRxns;
int numPatterns;
...
void recursiveCall(short *dirTemp, int level){
    ...
    int i, j, foundPattern;
    float newArrSize;
    /* Report on the number of configs found and cut */
    if (numCfgs % 100000 == 0 && dispFlag == 0) {
        mexPrintf("Number of configs: %d\n", numCfgs);
...
//                 break;
//             }
            if (unfeasiblePatterns[numPatterns * j + i] != 2 && dirTemp[j] != unfeasiblePatterns[numPatterns * j + i]){
                // Failed pattern matching. Move to the next pattern
                foundPattern = 0;
                break;
            }
...
//             totalNumConfigsCut += numConfigsCut;
    ...
    if (level > numRxns) {        
        ...
        if (numCfgs == cfgArrSize){
            ...
            printf("reallocating\n");
            newArrSize = SCALING_FACTOR * cfgArrSize;
            cfgArrSize = (int) newArrSize;
